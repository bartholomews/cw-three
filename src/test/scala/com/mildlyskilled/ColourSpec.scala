...
class ColourSpec extends DefaultHarness {

  val colour = Colour(0.45.toFloat, .20.toFloat, .10.toFloat)
  val colour1 = Colour(0.30.toFloat, .20.toFloat, .10.toFloat)
  val colorObject = Colour
  val white = Colour(23,2,3)
  ...
  it should "be 'addable' to another colour" in {
    colour + colour1 should be(Colour(0.75.toFloat,0.4.toFloat,0.2.toFloat))
  }

  it should "be 'subtractable' from another color" in {
    colour - colour1 should be(Colour(0.14999998.toFloat,0.0.toFloat,0.0.toFloat))
  }

  it should "be 'multipliable' by a float " in {
    colour * 2 should be(Colour(0.90.toFloat,0.40.toFloat,0.20.toFloat))
  }
  ...
  "The Colour object" should "Have the primary, secondary colours as well as black and white" in {
    colorObject.black should be(Colour(0,0,0))
    colorObject.white should be(Colour(1,1,1))
    colorObject.white should be(Colour(1,1,1))
    colorObject.red should be(Colour(1, 0, 0))
    colorObject.blue should be(Colour(0,0,1))
    colorObject.green should be(Colour(0,1,0))
  ...
  it should "derive a colour object given an RGB value" in {
    colorObject.fromRGB(colour.rgb) should be(Colour(colour.rgbTuple._1, colour.rgbTuple._2, colour.rgbTuple._3))
  }
}
